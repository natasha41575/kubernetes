...
func setUpPreemptionTestWithContext(t *testing.T, testCtx *testutils.TestContext, idx int, nodeCPUCapacity, otherPodCPURequest, deferredPodCPURequest, deferredPodCPUAllocated string) (*v1.Pod, *v1.Pod, *v1.Pod) {
	cs := testCtx.ClientSet
...
func TestDeferredResizeQueueingHints(t *testing.T) {
	...
	for idx, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			pod, other, irrelevant := setUpPreemptionTestWithContext(t, testCtx, idx, nodeCap, otherReq, defReq, "50m")
...
func TestDeferredResizeNodePreemptionPolicy(t *testing.T) {
	...
	t.Run("disabling node preemption policy removes pod from scheduling queue entirely", func(t *testing.T) {
		pod, other, irrelevant := setUpPreemptionTestWithContext(t, testCtx, 0, "200m", "200m", "100m", "50m")

		cs := testCtx.ClientSet
		queue := testCtx.Scheduler.SchedulingQueue

		// Verify pod is initially in queue
		...
		// Verify pod is removed from queue entirely (not in active, backoff, or unschedulable)
		err = wait.PollUntilContextTimeout(testCtx.Ctx, 50*time.Millisecond, 5*time.Second, true, func(context.Context) (bool, error) {
			_, found := queue.GetPod(pod.Name, pod.Namespace, nil)
			return !found, nil
		})
		if err != nil {
			t.Fatalf("Expected pod to be deleted from scheduling queue entirely after preemption disabled on node")
		}

		testutils.CleanupPods(testCtx.Ctx, cs, t, []*v1.Pod{pod, other, irrelevant})
	})

	t.Run("enabling node preemption policy adds unqueued pod back to scheduling queue", func(t *testing.T) {
		pod, other, irrelevant := setUpPreemptionTestWithContext(t, testCtx, 1, "200m", "200m", "100m", "50m")

		cs := testCtx.ClientSet
		queue := testCtx.Scheduler.SchedulingQueue

		nodeName := pod.Spec.NodeName
		// Disable preemption on node to remove pod from queue entirely
		n, err := cs.CoreV1().Nodes().Get(testCtx.Ctx, nodeName, metav1.GetOptions{})
		if err != nil {
			t.Fatalf("Failed to get node: %v", err)
		}
		...
		if err != nil {
			t.Fatalf("Expected pod to be deleted from scheduling queue entirely")
		}
		...
		// Verify unqueued pod is added back to scheduling queue (will be popped and parked in UnschedulablePods)
		err = wait.PollUntilContextTimeout(testCtx.Ctx, 50*time.Millisecond, 5*time.Second, true, func(context.Context) (bool, error) {
			_, found := queue.GetPod(pod.Name, pod.Namespace, nil)
			return found, nil
		})
		if err != nil {
			t.Fatalf("Expected unqueued pod to be added back to scheduling queue after preemption enabled on node")
		}
	...
	t.Run("irrelevant node preemption policy change ignores queue", func(t *testing.T) {
		pod, other, irrelevant := setUpPreemptionTestWithContext(t, testCtx, 2, "200m", "200m", "100m", "50m")
...
func TestDeferredResizeQueueingHandlers(t *testing.T) {
	...
	t.Run("AddPod handler ignores existing deferred pod on scheduler startup when preemption is disabled", func(t *testing.T) {
		...
		nodeObject.Spec.PodPreemptionPolicy = &v1.NodePodPreemptionPolicy{
			DisableResizePreemption: []string{"test-policy"},
		}
		if _, err := cs.CoreV1().Nodes().Create(testCtx.Ctx, nodeObject, metav1.CreateOptions{}); err != nil {
			t.Fatalf("Failed to create node: %v", err)
		...
		if err := testutils.WaitForNodesInCache(testCtx.Ctx, testCtx.Scheduler, 1); err != nil {
			t.Fatalf("Failed to wait for node in cache: %v", err)
		}

		queue := testCtx.Scheduler.SchedulingQueue
		time.Sleep(300 * time.Millisecond)
		if _, found := queue.GetPod(pod.Name, pod.Namespace, nil); found {
			t.Fatalf("Expected pod on node with preemption disabled not to be enqueued on startup")
		}
	...
	t.Run("UpdateNode handler removes pod from queue when node preemption policy transitions to disabled", func(t *testing.T) {
		featuregatetesting.SetFeatureGatesDuringTest(t, utilfeature.DefaultFeatureGate, featuregatetesting.FeatureOverrides{
			features.InPlacePodVerticalScaling:                    true,
			features.InPlacePodVerticalScalingSchedulerPreemption: true,
		})
		...
		// Update node spec to disable preemption
		nodeObject, err = cs.CoreV1().Nodes().Get(testCtx.Ctx, nodeObject.Name, metav1.GetOptions{})
		if err != nil {
			t.Fatalf("Failed to get node: %v", err)
		}
		nodeObject.Spec.PodPreemptionPolicy = &v1.NodePodPreemptionPolicy{
		...
		if _, err := cs.CoreV1().Nodes().Update(testCtx.Ctx, nodeObject, metav1.UpdateOptions{}); err != nil {
			t.Fatalf("Failed to update node preemption policy: %v", err)
		}
		...
		if err != nil {
			t.Fatalf("Expected pod to be removed from the scheduling queue when preemption is disabled on node")
		}

		testutils.CleanupPods(testCtx.Ctx, cs, t, []*v1.Pod{pod})
	})

	t.Run("UpdateNode handler enqueues pod when node preemption policy transitions to enabled", func(t *testing.T) {
		featuregatetesting.SetFeatureGatesDuringTest(t, utilfeature.DefaultFeatureGate, featuregatetesting.FeatureOverrides{
			features.InPlacePodVerticalScaling:                    true,
			features.InPlacePodVerticalScalingSchedulerPreemption: true,
		})
		...
		// Create node with preemption disabled
		nodeObject := st.MakeNode().Name("node1").Capacity(map[v1.ResourceName]string{
			v1.ResourcePods: "32",
			v1.ResourceCPU:  "200m",
		}).Obj()
		...
		// Update node spec to enable preemption
		nodeObject, err = cs.CoreV1().Nodes().Get(testCtx.Ctx, nodeObject.Name, metav1.GetOptions{})
		if err != nil {
			t.Fatalf("Failed to get node: %v", err)
		}
		nodeObject.Spec.PodPreemptionPolicy = nil
		if _, err := cs.CoreV1().Nodes().Update(testCtx.Ctx, nodeObject, metav1.UpdateOptions{}); err != nil {
			t.Fatalf("Failed to clear node preemption policy: %v", err)
		}

		// Verify the pod was enqueued automatically by the UpdateNode event handler!
		var found bool
		err = wait.PollUntilContextTimeout(testCtx.Ctx, 100*time.Millisecond, 2*time.Second, true, func(ctx context.Context) (bool, error) {
			_, found = queue.GetPod(pod.Name, pod.Namespace, nil)
			return found, nil
		...
		if err != nil || !found {
			t.Fatalf("Expected pod to be enqueued automatically by UpdateNode handler on preemption enablement")
		}
