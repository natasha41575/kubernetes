...
func TestDeferredResizeNodePreemptionPolicyFallback(t *testing.T) {
	t.Run("disabling node preemption policy removes pod from scheduling queue entirely", func(t *testing.T) {
		testCtx, pod, other, irrelevant := setUpPreemptionTest(t, "200m", "200m", "100m", "50m")
		defer testCtx.SchedulerCloseFn()
		...
		// Disable preemption on assigned node1
		n, err := cs.CoreV1().Nodes().Get(testCtx.Ctx, "node1", metav1.GetOptions{})
		if err != nil {
		...
		if _, err := cs.CoreV1().Nodes().Update(testCtx.Ctx, n, metav1.UpdateOptions{}); err != nil {
			t.Fatalf("Failed to update node1 to disable preemption: %v", err)
		}
		...
		if err != nil {
			t.Fatalf("Expected pod to be deleted from scheduling queue entirely after preemption disabled on node1")
		}
	...
	t.Run("enabling node preemption policy adds unqueued pod back to scheduling queue", func(t *testing.T) {
		...
		// Disable preemption on node1 to remove pod from queue entirely
		n, err := cs.CoreV1().Nodes().Get(testCtx.Ctx, "node1", metav1.GetOptions{})
		if err != nil {
			t.Fatalf("Failed to get node1: %v", err)
		}
		...
		// Now re-enable preemption on node1 (clear DisableResizePreemption)
		n, err = cs.CoreV1().Nodes().Get(testCtx.Ctx, "node1", metav1.GetOptions{})
		if err != nil {
			t.Fatalf("Failed to get node1: %v", err)
		}
		...
		if _, err := cs.CoreV1().Nodes().Update(testCtx.Ctx, n, metav1.UpdateOptions{}); err != nil {
			t.Fatalf("Failed to update node1 to enable preemption: %v", err)
		}
		...
		if err != nil {
			t.Fatalf("Expected unqueued pod to be added back to scheduling queue after preemption enabled on node1")
		}
	...
	t.Run("irrelevant node preemption policy change ignores queue", func(t *testing.T) {
		...
		n, err := cs.CoreV1().Nodes().Get(testCtx.Ctx, "node2", metav1.GetOptions{})
		if err != nil {
