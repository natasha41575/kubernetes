...
func doPodResizeDeferredPreemptionTests(f *framework.Framework) {
	ginkgo.It("preempt a lower-priority pod to satisfy a deferred resize", func(ctx context.Context) {
		...
		// Victim pod: 60% of available CPU, low priority
		victimCPU := resource.NewMilliQuantity(int64(float64(availableMillis)*0.6), resource.DecimalSI)
		// Preemptor pod: 30% of available CPU, high priority
		preemptorCPU := resource.NewMilliQuantity(int64(float64(availableMillis)*0.3), resource.DecimalSI)
		...
		ginkgo.By(fmt.Sprintf("Create victim pod '%s' (60%% CPU) and preemptor pod '%s' (30%% CPU) in batch", victim.Name, preemptor.Name))
		createdPods := podClient.CreateBatch(ctx, []*v1.Pod{victim, preemptor})
		victim = createdPods[0]
		preemptor = createdPods[1]

		// Resize preemptor to 60% CPU (needs 30% more, only 10% remains free on node)
		preemptorResizedCPU := resource.NewMilliQuantity(int64(float64(availableMillis)*0.6), resource.DecimalSI)
		...
		}`, preemptorResizedCPU.MilliValue(), preemptorResizedCPU.MilliValue())

		ginkgo.By(fmt.Sprintf("Resize high-priority preemptor pod '%s' to 60%% CPU (exceeds node capacity, should defer)", preemptor.Name))
		...
		ginkgo.By(fmt.Sprintf("Verify that preemptor pod '%s' immediately has PodResizePreemptionDisabled condition and reason while node policy disables resize preemption", preemptor.Name))
		preemptor, err = f.ClientSet.CoreV1().Pods(preemptor.Namespace).Get(ctx, preemptor.Name, metav1.GetOptions{})
		framework.ExpectNoError(err, "failed to get preemptor pod after deferred resize")

		var preemptionDisabledCond *v1.PodCondition
		for i, cond := range preemptor.Status.Conditions {
			if cond.Type == v1.PodResizePreemptionDisabled {
				preemptionDisabledCond = &preemptor.Status.Conditions[i]
				break
			}
		}
		gomega.Expect(preemptionDisabledCond).NotTo(gomega.BeNil(), "preemptor pod should have PodResizePreemptionDisabled condition")
		gomega.Expect(preemptionDisabledCond.Status).To(gomega.Equal(v1.ConditionTrue), "PodResizePreemptionDisabled status should be True")
		gomega.Expect(preemptionDisabledCond.Reason).To(gomega.Equal(v1.PodReasonPreemptionDisabledByNodePolicy), "PodResizePreemptionDisabled reason should be PreemptionDisabledByNodePolicy")
		...
		ginkgo.By(fmt.Sprintf("Verify that victim pod '%s' gets preempted/evicted after enabling resize preemption", victim.Name))
		...
		expected := []podresize.ResizableContainerInfo{
			{
				Name:      "cb",
				Resources: &cgroups.ContainerResources{CPUReq: preemptorResizedCPU.String(), CPULim: preemptorResizedCPU.String()},
			},
		}
		resizedPreemptor := podresize.WaitForPodResizeActuation(ctx, f, podClient, preemptor, expected)
		podresize.ExpectPodResized(ctx, f, resizedPreemptor, expected)
	})
}
