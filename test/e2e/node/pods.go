...
var _ = SIGDescribe("Pods Extended", func() {
	...
	ginkgo.Describe("Pod Generation", func() {
		...
		ginkgo.It("pod generation should start at 1 and increment per update", func(ctx context.Context) {
			...
			ginkgo.By("updating podspec to trigger generation bump")
			...
			ginkgo.By("pod generation set by client should be ignored")
			podClient.Update(ctx, name, func(pod *v1.Pod) {
				pod.SetGeneration(5)
			})
			pods, err = podClient.List(ctx, options)
		...
		ginkgo.It("mirror pods should not have generation populated", func(ctx context.Context) {
			...
			ginkgo.By("updating podspec, generation should remain unpopulated")
			labelFooName := "foo-" + string(uuid.NewUUID())
			podClient.Update(ctx, name, func(pod *v1.Pod) {
				pod.Spec.Tolerations = []v1.Toleration{
					{
