...
// checkpointV1 represents the legacy JSON structure preivously used for resource checkpointing.
...
var _ checkpointmanager.Checkpoint = &checkpointV1{}
...
func migrateV1ToV2(data string) (*v1.PodList, error) {
	...
	for podUID, entry := range checkpointData.Entries {
		...
		// On startup, UpdatePodFromStoredState uses ContainerIter to search across both InitContainers and Containers
		// matching by container name, ensuring init container allocations are correctly restored. Once the pod is
