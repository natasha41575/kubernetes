...
func TestStateMemory_EmptyDirVolumeLimits(t *testing.T) {
	...
	// 1. Get on nonexistent pod should return (nil, false)
	qty, exists := state.GetEmptyDirVolumeLimit(podUID, volName)
	assert.Nil(t, qty)
	assert.False(t, exists)

	// 2. Set volume limit on nonexistent pod should implicitly initialize the pod and insert it
	targetLimit := resource.MustParse("256Mi")
	err := state.SetEmptyDirVolumeLimit(podUID, volName, &targetLimit)
	require.NoError(t, err)

	// 3. Get volume limit should return the parsed value and true
	qty, exists = state.GetEmptyDirVolumeLimit(podUID, volName)
	require.True(t, exists)
	require.NotNil(t, qty)
	assert.True(t, targetLimit.Equal(*qty))

	// 4. Get on existing pod with nonexistent volume should return (nil, false)
	...
	// 5. Returned quantity should be a deep copy (mutability check)
	...
	// 6. Set another volume on the same pod should keep existing limits intact
