...
func verifyPodResourceAllocation(t *testing.T, expected, actual *PodResourceInfoMap, msgAndArgs string) {
	for podUID, expectedPodInfo := range *expected {
		...
		require.Equal(t, len(expectedPodInfo.ContainerResources), len(actualPodInfo.ContainerResources), msgAndArgs)
		...
		if expectedPodInfo.EmptyDirVolumeLimits == nil {
			...
			require.Equal(t, len(expectedPodInfo.EmptyDirVolumeLimits), len(actualPodInfo.EmptyDirVolumeLimits), msgAndArgs)
