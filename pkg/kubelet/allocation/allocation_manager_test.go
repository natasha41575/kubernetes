	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.allocated != nil {
				allocationManager.(*manager).allocated.SetPod(logger, test.allocated)
			}
...
func TestAllocationManager_Upgrade_CheckpointMigration(t *testing.T) {
	...
	require.NotNil(t, checkpointV2.PodList)
	require.Len(t, checkpointV2.PodList.Items, 1)
	checkpointPod := &checkpointV2.PodList.Items[0]
