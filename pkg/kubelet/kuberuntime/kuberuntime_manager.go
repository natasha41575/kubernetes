	...
	if utilfeature.DefaultFeatureGate.Enabled(features.InPlacePodVerticalScalingMemoryBackedVolumes) {
		...
		for _, vol := range pod.Spec.Volumes {
			if vol.EmptyDir != nil && vol.EmptyDir.Medium == v1.StorageMediumMemory {
				...
				if actuatedSize == nil && (newSize == nil || newSize.Value() == 0) {
					continue
				}
				if actuatedSize != nil && newSize != nil && actuatedSize.Equal(*newSize) {
					continue
				}

				// If they differ, mark for resize.
				isUpsize := false
				if newSize != nil && actuatedSize != nil && newSize.Cmp(*actuatedSize) > 0 {
					isUpsize = true
				} else if actuatedSize == nil && newSize != nil {
					isUpsize = true
				}
...
func (m *kubeGenericRuntimeManager) doPodResizeAction(ctx context.Context, pod *v1.Pod, podStatus *kubecontainer.PodStatus, podContainerChanges podActions) *kubecontainer.SyncResult {
	...
	updateVolumeSize := func(volumes []v1.Volume, isUpsize bool) error {
		if utilfeature.DefaultFeatureGate.Enabled(features.InPlacePodVerticalScalingMemoryBackedVolumes) {
			for _, vol := range volumes {
				...
				klog.V(2).InfoS("Resizing volume", "volume", vol.Name, "pod", klog.KObj(pod), "isUpsize", isUpsize)
	...
	// 1. Volume Downsize
	if utilfeature.DefaultFeatureGate.Enabled(features.InPlacePodVerticalScalingMemoryBackedVolumes) {
		if len(podContainerChanges.VolumesToDownsize) > 0 {
			if err := updateVolumeSize(podContainerChanges.VolumesToDownsize, false); err != nil {
				resizeResult.Fail(kubecontainer.ErrResizePodInPlace, err.Error())
				return resizeResult
			}
		}
	...
	// 3. Volume upsize
	if utilfeature.DefaultFeatureGate.Enabled(features.InPlacePodVerticalScalingMemoryBackedVolumes) {
		if len(podContainerChanges.VolumesToUpsize) > 0 {
			if err := updateVolumeSize(podContainerChanges.VolumesToUpsize, true); err != nil {
				resizeResult.Fail(kubecontainer.ErrResizePodInPlace, err.Error())
				return resizeResult
			}
		}
