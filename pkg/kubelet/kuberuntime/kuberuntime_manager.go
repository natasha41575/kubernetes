...
func (m *kubeGenericRuntimeManager) doPodResizeAction(pod *v1.Pod, podContainerChanges podActions) *kubecontainer.SyncResult {
	...
	if podResources == nil {
		...
		metrics.PodResizeDurationMilliseconds.WithLabelValues("false").Observe(float64(time.Since(start).Milliseconds()))
		return resizeResult
	}
	...
	metrics.PodResizeDurationMilliseconds.WithLabelValues("true").Observe(float64(time.Since(start).Milliseconds()))
	return resizeResult
}
