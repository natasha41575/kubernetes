...
func TestComputeVolumeResizeAction(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			testName:       "Upsize: new limit specified, no actuated limit",
			enableGate:     true,
			actuatedLimit:  nil,
			newLimit:       resource.NewQuantity(100, resource.BinarySI),
			volMedium:      v1.StorageMediumMemory,
		...
		{
			testName:       "Downsize: limit removed (new limit nil)",
			enableGate:     true,
			actuatedLimit:     resource.NewQuantity(100, resource.BinarySI),
			newLimit:       nil,
			volMedium:      v1.StorageMediumMemory,
...
func TestDoPodResizeAction_Volumes(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.testName, func(t *testing.T) {
			...
			require.Equal(t, len(tc.expectedCalls), len(helper.resizeCalls), "number of ResizeEphemeralVolume calls")
