...
func (m *manager) SetPodResizePendingCondition(podUID types.UID, reason, message string) {
	...
	oldConditions, found := m.podResizeConditions[podUID]
	if !found {
		oldConditions = podResizeConditions{}
	}

	m.podResizeConditions[podUID] = podResizeConditions{
		PodResizePending:    updatedPodResizeCondition(v1.PodResizePending, oldConditions.PodResizePending, reason, message),
...
func (m *manager) SetPodResizeInProgressCondition(podUID types.UID, reason, message string) {
	...
	m.podResizeConditions[podUID] = podResizeConditions{
		PodResizeInProgress: updatedPodResizeCondition(v1.PodResizeInProgress, oldConditions.PodResizeInProgress, reason, message),
...
func (m *manager) ClearPodResizePendingCondition(podUID types.UID) {
	...
	_, found := m.podResizeConditions[podUID]
	if !found {
		return
	}

	newConditions := podResizeConditions{
		PodResizePending:    nil,
		PodResizeInProgress: m.podResizeConditions[podUID].PodResizeInProgress,
	}

	m.podResizeConditions[podUID] = newConditions
}
...
func (m *manager) ClearPodResizeInProgressCondition(podUID types.UID) {
	...
	if _, found := m.podResizeConditions[podUID]; !found {
		return
	}
