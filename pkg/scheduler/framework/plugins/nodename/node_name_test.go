...
func TestNodeName_PreFilter(t *testing.T) {
	tests := []struct {
		...
		{
			name: "assigned pod with nodeName set, feature enabled, prefilter restricts to assigned node",
			pod:  st.MakePod().Node("foo").Condition(v1.PodResizePending, v1.ConditionTrue, v1.PodReasonDeferred).Obj(),
		...
		{
			name: "assigned pod with nodeName set, feature disabled, prefilter returns nil",
			pod:  st.MakePod().Node("foo").Obj(),
			inPlacePodVerticalScalingSchedulerPreemptionEnabled: false,
		},
		{
			name: "unassigned pod without nodeName set, feature enabled, prefilter returns nil",
			pod:  st.MakePod().Obj(),
			inPlacePodVerticalScalingSchedulerPreemptionEnabled: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, ctx := ktesting.NewTestContext(t)
			p, err := New(ctx, nil, nil, feature.Features{
				EnableInPlacePodVerticalScalingSchedulerPreemption: tt.inPlacePodVerticalScalingSchedulerPreemptionEnabled,
			})
			if err != nil {
				t.Fatalf("creating plugin: %v", err)
			}
