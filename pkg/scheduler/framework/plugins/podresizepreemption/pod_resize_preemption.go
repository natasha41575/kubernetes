...
type PodResizePreemption struct {
	enableInPlacePodVerticalScalingSchedulerPreemption bool
}

var _ fwk.PreFilterPlugin = &PodResizePreemption{}
var _ fwk.FilterPlugin = &PodResizePreemption{}
var _ fwk.EnqueueExtensions = &PodResizePreemption{}
var _ fwk.PermitPlugin = &PodResizePreemption{}

const (
	Name      = names.PodResizePreemption
	ErrReason = "node(s) had resize preemption disabled"
)

func (pl *PodResizePreemption) Name() string {
	return Name
}

// EventsToRegister returns the possible events that may make a Pod
// failed by this plugin schedulable.
func (pl *PodResizePreemption) EventsToRegister(_ context.Context) ([]fwk.ClusterEventWithHint, error) {
...
func (pl *PodResizePreemption) isSchedulableAfterNodeChange(logger klog.Logger, pod *v1.Pod, oldObj, newObj interface{}) (fwk.QueueingHint, error) {
	...
	if oldNodeDisabled && !newNodeDisabled {
		logger.V(3).Info("Node preemption policy changed from disabled to enabled, queueing pod", "pod", klog.KObj(pod), "node", newNode.Name)
...
func (pl *PodResizePreemption) isSchedulableAfterNodeAdd(logger klog.Logger, pod *v1.Pod, oldObj, newObj interface{}) (fwk.QueueingHint, error) {
	if !pl.enableInPlacePodVerticalScalingSchedulerPreemption || !resource.IsPodResizeDeferred(pod) {
		return fwk.QueueSkip, nil
	}
	...
	if node.Spec.PodPreemptionPolicy == nil || len(node.Spec.PodPreemptionPolicy.DisableResizePreemption) == 0 {
		logger.V(3).Info("Node added with preemption enabled, queueing pod", "pod", klog.KObj(pod), "node", node.Name)
		return fwk.Queue, nil
	}
...
func (pl *PodResizePreemption) PreFilter(ctx context.Context, state fwk.CycleState, pod *v1.Pod, nodes []fwk.NodeInfo) (*fwk.PreFilterResult, *fwk.Status) {
	if !pl.enableInPlacePodVerticalScalingSchedulerPreemption || !resource.IsPodResizeDeferred(pod) {
...
func (pl *PodResizePreemption) PreFilterExtensions() fwk.PreFilterExtensions {
	return nil
}

func (pl *PodResizePreemption) Filter(ctx context.Context, _ fwk.CycleState, pod *v1.Pod, nodeInfo fwk.NodeInfo) *fwk.Status {
	if !pl.enableInPlacePodVerticalScalingSchedulerPreemption || !resource.IsPodResizeDeferred(pod) {
		return nil
	}
...
func (pl *PodResizePreemption) Permit(ctx context.Context, state fwk.CycleState, p *v1.Pod, nodeName string) (*fwk.Status, time.Duration) {
